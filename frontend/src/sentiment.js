...
console.log(tweetsLoader("python"))
...
const tweetSentiment = (input) => {
    const output = {};
    const tweets = tweetsLoader(input);
    console.log(tweets)
    if(tweets !== undefined) {
        for(let i = 1; i < tweets.length; i++) {

            const result =  quickstart(tweets[i]);
            output[i] = result
            console.log(output);
        }
    }
...
// console.log(tweetSentiment("trump"))
